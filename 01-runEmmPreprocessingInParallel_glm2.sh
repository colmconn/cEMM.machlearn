...
if [[ "$#" -gt 0 ]] ; then
    ...
    # When you change between the list of baseline subjects only that
    # you ensure that timepointLimiter is set to baselineOnly a few
    # lines below.  This is because the alignment parameters for the
    # baseline subjects and follow-up subjects are stored in different
    # files. The file that is read in is ultimately controlled by the
    # timepointLimiter variable

    ## baseline only subjects (note the presence of A to limit to only baseline subjects)
    subjects=$( cd ../data/ ; ls -d [0-9][0-9][0-9]_A [0-9][0-9][0-9]_A2 2> /dev/null | grep -v 999 )

    ## follow-up only subjects(note the presence of {C,D} to limit to only follow-up subjects)
    ## subjects=$( cd ../data/ ; ls -d [0-9][0-9][0-9]_{C,D} [0-9][0-9][0-9]_{C,D}2 2> /dev/null | grep -v 999 )
fi

## ensure that the value of thei variable matches the list of subjects
## included for analysis above!
...
preprocessingScript=${subject}.afniEmmPreprocess.glm2.$scriptExt.csh
...
threeDeeDeconScript=${subject}.afniEmmPreprocess.glm2.$scriptExt.csh
deconPrefix=glm2.
afni_proc.py -subj_id ${subject}										\\
	     -write_3dD_script \${threeDeeDeconScript}								\\
	     ...
	     -mask_apply group											\\
	     -regress_stim_times 										\\
	     		$DATA/regressors/ml_regressors_glm2/1_screen.1D 					\\
			$DATA/regressors/ml_regressors_glm2/2_oval.1D 						\\
			$DATA/regressors/ml_regressors_glm2/3_fear.1D 						\\
			$DATA/regressors/ml_regressors_glm2/4_happy.1D 						\\
			$DATA/regressors/ml_regressors_glm2/5_sad.1D 						\\
			$DATA/regressors/ml_regressors_glm2/6_blank.1D 						\\
	     -regress_stim_labels Screen Oval Fear Happy Sad Blank						\\
	     -regress_basis 'SPMG1'										\\
	     -regress_stim_types IM IM IM IM IM IM                                            			\\
...
if [[ -f \${threeDeeDeconScript} ]] ; then 
    cd \${outputDir}

    ## for some reason the afni_proc command above doe snot copy regressors to the stimuli directory, so do it here
    cp $DATA/regressors/ml_regressors_glm2/1_screen.1D \\
       $DATA/regressors/ml_regressors_glm2/2_oval.1D \\
       $DATA/regressors/ml_regressors_glm2/3_fear.1D  \\
       $DATA/regressors/ml_regressors_glm2/4_happy.1D \\
       $DATA/regressors/ml_regressors_glm2/5_sad.1D \\
    ...
    tcsh -xef ../\${threeDeeDeconScript}

    xmat_regress=\${deconPrefix}X.xmat.1D 
	...
	if [[ \$numberOfCensoredVolumes -gt \$cutoff ]] ; then 
		...
		00_DO_NOT_ANALYSE_${subject}_\${excessiveMotionThresholdPercentage}percent_glm2.txt
	...
	touch 00_DO_NOT_ANALYSE_${subject}_\${excessiveMotionThresholdPercentage}percent_glm2.txt
    fi
